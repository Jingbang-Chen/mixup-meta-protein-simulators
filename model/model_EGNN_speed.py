...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, act_fn=nn.SiLU(), residual=True, attention=False, normalize=False, coords_agg='mean', tanh=False, phm_dim = 4, change_coord = True):
        ...
        self.edge_mlp = nn.Sequential(
            ...
            act_fn,
            PHMLinear(hidden_nf, hidden_nf, bias=True, phm_dim=phm_dim),
            act_fn
        )

        self.node_mlp = nn.Sequential(
            PHMLinear(hidden_nf*2 + input_nf, hidden_nf, bias=True, phm_dim=phm_dim),
            act_fn,
        ...
        if self.change_coord:
            coord_mlp_edge = []
            coord_mlp_edge.append(PHMLinear(hidden_nf*2, hidden_nf, bias=True, phm_dim=phm_dim))
            ...
            coord_mlp_point = []
            coord_mlp_point.append(PHMLinear(hidden_nf*2, hidden_nf, bias=True, phm_dim=phm_dim))
    ...
    def coord2radial(self, edge_index, coord):
        ...
        npr = radial.detach().cpu().numpy()
        if(np.any(np.isnan(npr))):
            print("radial fault?!")
