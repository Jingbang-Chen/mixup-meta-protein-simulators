...
class E_GCL(nn.Module):
    ...
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, act_fn=nn.SiLU(), residual=True, attention=False, normalize=False, coords_agg='mean', tanh=False, phm_dim = 4, change_coord = True):
        ...
        self.phm_node_layer = PHMLinear(hidden_nf + input_nf, hidden_nf, bias=True, phm_dim=phm_dim)
        ...
        if self.change_coord:
            self.phm_coord_edge = PHMLinear(hidden_nf, hidden_nf, bias=True, phm_dim=phm_dim)
            self.phm_coord_point = PHMLinear(hidden_nf, hidden_nf, bias=True, phm_dim=phm_dim)
...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, hidden_nf, out_node_nf, in_edge_nf=0, device='cpu', act_fn=nn.SiLU(), n_layers=4, residual=True, attention=False, normalize=False, tanh=False, step=1):
        ...
        self.embedding_point = nn.Linear(self.hidden_nf + 3, self.hidden_nf)
        self.embedding_in2 = nn.Linear(self.hidden_nf * 3 + 3, self.hidden_nf)
    ...
    def forward(self, h, x, edges, edge_attr, prompt, pos_diff, point_h, point_acc):
        point_num = h.shape[0]
        # print(h.shape, prompt.repeat(point_num, 1).shape, pos_diff.shape)
        point_prompt = torch.cat([point_h, point_acc], dim=1)
        point_prompt = self.embedding_point(point_prompt)
    ...
    def first_layer(self, h, x, edges, edge_attr, prompt, pos_diff, point_h, point_acc):
        point_num = h.shape[0]
        # print(h.shape, prompt.repeat(point_num, 1).shape, pos_diff.shape)
        point_prompt = torch.cat([point_h, point_acc], dim=1)
        point_prompt = self.embedding_point(point_prompt)
